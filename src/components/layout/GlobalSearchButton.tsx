...
export function GlobalSearchButton(){
  ...
  useEffect(()=>{
    ...
    if(!open){ setQ(""); setResults({meetings:[], people:[], topics:[]}); setError(null); setLoading(false); abortRef.current?.abort() }
  },[open])
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-[12vh] sm:pt-[20vh] bg-background/60 backdrop-blur-sm p-4" onClick={()=>setOpen(false)}>
          <div className="w-full max-w-xl rounded-2xl border border-border bg-card shadow-2xl overflow-hidden animate-scale-in" onClick={e=>e.stopPropagation()}>
            ...
            <div className="max-h-[60vh] overflow-auto p-2">
              ...
              {q.trim() && q.trim().length>100 && (
                <p className="p-4 text-sm text-muted-foreground text-center">Query must be 1-100 characters</p>
              )}
              {!q.trim() && !error && (
                <div className="p-6 text-center">
                  <p className="text-sm text-muted-foreground">Type to search</p>
                  <p className="text-xs text-muted-foreground mt-1">Meetings · People · Topics (agenda/description)</p>
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        {results.meetings.map(m=>(
                          <Link key={m.id} href={`/meetings/${m.id}`} onClick={()=>setOpen(false)} className="flex gap-3 p-3 rounded-xl hover:bg-muted transition-colors">
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        {results.people.map(p=>(
                          <Link key={p.id} href={`/meetings/${p.meetingId}`} onClick={()=>setOpen(false)} className="flex gap-3 p-3 rounded-xl hover:bg-muted transition-colors">
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        {results.topics.map(t=>(
                          <Link key={t.id} href={`/meetings/${t.meetingId}`} onClick={()=>setOpen(false)} className="flex gap-3 p-3 rounded-xl hover:bg-muted transition-colors">
