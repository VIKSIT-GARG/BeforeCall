...
function AttendeeCard({a}:{a:any}){ const [open,setOpen]=useState(false); return <Card className="overflow-hidden"><CardContent className="pt-6"><div className="flex gap-4"><Avatar className="h-12 w-12 shrink-0"><AvatarFallback>{getInitials(a.name)}</AvatarFallback></Avatar><div className="flex-1 min-w-0"><div className="flex flex-wrap items-center gap-2"><h4 className="font-semibold">{a.name}</h4><Badge variant={a.confidence==='verified'?"default": a.confidence==='inferred'?"secondary":"outline"}>{a.confidence}</Badge>{a.company && <Badge variant="outline">{a.company}</Badge>}</div><p className="text-sm text-muted-foreground">{a.role} {a.company? `· ${a.company}`:""}</p><p className="text-sm mt-2 leading-relaxed">{a.relevantBackground}</p><p className="text-sm text-primary mt-2 font-medium bg-primary/5 p-2.5 rounded-lg border border-primary/10">{a.whyTheyMatter}</p><Button variant="ghost" size="sm" className="mt-2 h-7 px-2" onClick={()=>setOpen(!open)}>{open?"Hide":"Show"} context <ChevronDown className={cn("h-3 w-3 ml-1 transition-transform",open&&"rotate-180")}/></Button>{open && <div className="mt-3 text-sm space-y-2 animate-fade-in"><Separator/><SourceChips sources={a.sources||[]}/></div>}</div></div></CardContent></Card>}
