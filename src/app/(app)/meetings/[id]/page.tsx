...
export default function MeetingDetailPage() {
  ...
  const startResearch = async()=>{
    ...
    // Fake progressive updates while real API runs (real API would need SSE; we simulate)
    const timer = setInterval(()=> setProgress(p=> p && p.progress < 90 ? {...p, progress: p.progress + 7, message: p.message} as any : p ), 1200)
    try{
      const r = await fetch(`/api/research/${id}`, {method:'POST'})
      const d = await r.json()
      clearInterval(timer)
      if(!d.success) throw new Error(d.error||'Research failed')
      setProgress({stage:'complete', progress:100, message:'Research complete!'})
      await fetchMeeting()
    }catch(e:any){
      clearInterval(timer)
